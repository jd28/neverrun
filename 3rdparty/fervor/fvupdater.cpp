...
FvUpdater::FvUpdater(QObject *parent) : QObject(0)
{
    m_reply = 0;
#ifdef FV_GUI
    m_updaterWindow = 0;
...
void FvUpdater::showUpdaterWindowUpdatedWithCurrentUpdateProposal()
{
    ...
    // Create a new window
    m_updaterWindow = new FvUpdateWindow(this, NULL, skipVersionAllowed, remindLaterAllowed);
    m_updaterWindow->UpdateWindowWithCurrentProposedUpdate();
    m_updaterWindow->adjustSize();
    m_updaterWindow->move(QApplication::desktop()->screen()->rect().center() - m_updaterWindow->rect().center());
...
void FvUpdater::setWidgetParent(QMainWindow *widget) {
